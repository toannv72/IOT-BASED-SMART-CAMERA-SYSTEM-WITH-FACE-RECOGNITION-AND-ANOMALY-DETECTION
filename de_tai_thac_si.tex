...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        \node[box, below=1.0cm of gassensor] (doorlock) {Solenoid Door Lock\\(GPIO23 Control)};
        \node[box, below=1.0cm of doorlock] (buzzer) {Physical Alarm Buzzer\\(GPIO24 Output)};
        ...
        % Edge Border
        \node[container, fit=(camera) (gassensor) (doorlock) (buzzer) (roi) (anomaly) (buffer), label={[font=\footnotesize, text=blue]\textbf{Edge Processing Layer}}] (edge_group) {};
...
\begin{itemize}
    \item \textbf{Camera Thread Separation:} Each active camera feed is allocated a separate OS-level daemon thread running a generator loop. This ensures that a slow RTSP connection or frame drop on one channel does not block or latency-starve other processing channels.
    \item \textbf{In-Thread Sequential Pipeline Execution:} To avoid heavy thread contention and memory thrashing on the GPU/CPU device, the processing models (YOLOv5, MTCNN, FaceNet, and YOLOv8-fall/fire) are executed \textit{sequentially} on each frame retrieved from the camera thread's circular queue. PyTorch's execution context acts as a thread-safe serialization manager.
    \item \textbf{GPU/VRAM Shared Session (Singleton Pattern):} To prevent duplicate memory allocations that quickly trigger Out-Of-Memory (OOM) errors, the system loads only a single global instance of each AI model (YOLOv5, MTCNN, FaceNet, YOLOv8) into VRAM/system memory. Inference calls from multiple camera threads share these instances.
    \item \textbf{Dynamic Priority and Cooldowns (Lazy Loading):} Rather than running all detection models on every frame, the system employs a priority hierarchy. YOLOv5 ROI intrusion detection and YOLOv8 hazard detection run on every frame as lightweight first-tier monitors. The computationally heavier FaceNet embedding extraction is only triggered \textit{after} MTCNN cascade stages successfully detect a face. Cooldown variables (e.g., \texttt{face\_log\_cooldown} and \texttt{counter\_cooldown}) pause face matching and logging for configured durations once an identity is confirmed, saving massive CPU/GPU cycles.
    \item \textbf{Frame Rate Stabilization:} To prevent the edge device from running at 100\% utilization and overheating, a dynamic sleep loop is implemented. The thread worker calculates the elapsed processing time of the current frame and inserts a target sleep delay ($t_{delay} = \max(0.005, 0.04 - t_{elapsed})$) to throttle the local stream processing speed to a maximum of 25 FPS, reducing energy consumption and resource utilization.
\end{itemize}
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        % --- ACTORS ---
        \node[actor={System Administrator}] (admin) at (0, 1.5) {};
        \node[actor={Security Staff}] (staff) at (0, -1.5) {};
        
        % --- USE CASES (COLUMN 1: SHARED / READ ONLY) ---
        \node[usecase] (uc1) at (4.5, 2.5) {View Live\\Camera Stream};
        \node[usecase] (uc4) at (4.5, 0.2) {View System\\Event Logs};
        \node[usecase] (uc5) at (4.5, -2.1) {Receive Remote\\Telegram Alerts};
        
        % --- USE CASES (COLUMN 2: ADMIN ONLY / CONFIG) ---
        \node[usecase] (uc2) at (8.0, 2.5) {Register Face\\Embeddings};
        \node[usecase] (uc3) at (8.0, 0.2) {Configure ROI\\Polygons};
        \node[usecase] (uc6) at (8.0, -2.1) {Modify System\\Configurations};
